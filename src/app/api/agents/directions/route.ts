...
import type { ProjectBrief, StructuredRequirement } from "@/lib/agents/types";
...
export async function POST(request: NextRequest) {
  ...
  let project: ProjectBrief | undefined;
  try {
    ...
    if (body?.project && typeof body.project === "object") project = body.project as ProjectBrief;
  } catch {
  ...
  try {
    const result = await orchestrator.runDirections(requirement, project, guest.guestId);
